...
if (getArgumentValue === undefined) {
	photo_path_to_frames = "C:/Users/Danish/Desktop/MC A1/Mobile_Computing_Group14/uploads/";
} else {
    ...
    fs.readdir(photo_path, async function (err, items) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (path.extname(items[i]) === "") {
                console.log("Progress: ",i,"   -  ",items[i]);
                func_path = photo_path + items[i] + "/";
				if(fs.existsSync(func_path + "key_points.json")){
					console.log(func_path + "key_points.json"," is present, skipping this");
				} else {
					console.time('timer')
...
async function main() {
	...
	// 2.
	await cascading_images_pose_estimation(photo_path_to_frames, 1).then(r => console.log("Key Points generated Successfully."), () => console.log("Error occurred in generating keypoints."));
